...
- **Open `.md` from Explorer** — the portable `.exe` accepts a `.md` / `.markdown` / `.txt` path as its first argument. Windows must invoke the exact currently extracted executable as `"C:\path\to\MarkdownConverter.exe" "%1"`; `"%1"` is required and preserves spaces and non-ASCII characters. Right-click a Markdown file → *Open with* → *Choose another app* → browse to the current `MarkdownConverter.exe`. The portable ZIP does not install itself or update associations. If an association still targets an older build, replace it through **Settings → Apps → Default apps** and select the current extracted executable again.
